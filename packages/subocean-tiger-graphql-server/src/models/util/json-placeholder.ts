...
const apiBaseUrl = process.env.JSON_PLACEHOLDER_URL || "https://jsonplaceholder.typicode.com";
...
export async function createResource<T> (endpoint: string, body: string): Promise<T> {
  const response = await fetch(`${apiBaseUrl}${endpoint}`, {
    method: 'POST',
    headers: {
...
export async function updateResource<T> (endpoint: string, body: string): Promise<T> {
  const response = await fetch(`${apiBaseUrl}${endpoint}`, {
    method: 'PATCH',
    headers: {
      'Content-Type': 'application/json',
...
function getPaginationQueryParams (
  paginate?: PaginateOptions, slice?: SliceOptions
): QueryStringParams {
...
function getOperatorsQueryParams (operators?: OperatorOptions[]): QueryStringParams {
  const params = {} as QueryStringParams
...
export async function fetchPage<T> (
  endpoint: string, options?: PageQueryOptions
): Promise<Page<T>> {
  let url: string
