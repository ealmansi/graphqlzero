...
function Examples () {
  const [activeQueryId, setActiveQueryId] = useState('get-user-posts');
  const exampleQueries = getExampleQueries();
  const activeQuery = exampleQueries.find(({ id }) => id === activeQueryId) || exampleQueries[0];
  function buildClassName (id: string): string {
    return id === activeQueryId ? 'active' : '';
  }
  function buildOnClickHandler (id: string): ((event: React.MouseEvent) => void) {
    return () => { setActiveQueryId(id); }
  }
  return (
    <section id="examples" className="examples">
      ...
      <div className="columns">
        <div className="column left-column">
          <section id="example-query">
            ...
            <QueryDisplay exampleQuery={activeQuery}/>
          </section>
          {
            activeQuery.variables !== undefined ? (
              <section>
                <h1>Variables</h1>
                <VariablesDisplay exampleQuery={activeQuery}/>
              </section>
            ) : null
          }
          <section>
            <h1>Response</h1>
            {
              (activeQuery.type === 'query') ? (
                <Query query={gql`${activeQuery.query}`} variables={activeQuery.variables}>
                  {
                    ({ loading, error, data }: any) => {
                      return <ResponseDisplay {...{ loading, error, data }} />;
                    }
                  }
                </Query>
              ) : (
                <Mutation mutation={gql`${activeQuery.query}`} variables={activeQuery.variables}>
                  {
      ...
      <style jsx>{`
        ...
        .examples .columns .right-column ul {
          margin: 0;
          padding: 30px 0 10px 0;
          list-style: none;
        }
        .examples .columns .right-column ul li a {
          width: calc(100% - 20px);
          padding: 10px;
          display: inline-block;
          color: black;
        ...
        .examples .columns .right-column ul li.active a {
          background: lightgray;
        }
        .examples .columns .right-column ul li:hover a {
          background: lightgray;
        }
        ...
        .examples .columns .left-column section {
          padding-top: 20px;
          // padding-left: 20px;
        }
        @media screen and (max-width: 667px) {
...
function VariablesDisplay (props: { exampleQuery: ExampleQuery }) {
  const { exampleQuery } = props;
  const { variables } = exampleQuery;
  const variablesDisplay = JSON.stringify(variables, null, 2);
  const codeHtml = Prism.highlight(variablesDisplay, Prism.languages.json, 'json');
  return (
    <pre>
      <code dangerouslySetInnerHTML={{ __html: codeHtml }}></code>
      <style jsx>{`
...
function ResponseDisplay (props: any) {
